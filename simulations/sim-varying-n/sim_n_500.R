...
dists_wbb1_n500 <- dists_wbb2_n500 <- vector("list", total_sims)
dists_bob_n500 <- optimals_x_bob_n500 <- vector("list", total_sims)
...
for (sim in seq_len(total_sims)) {
  ...
  # Store the results
  dists_wbb1_n500[[sim]] <- sw2wbb1
  dists_wbb2_n500[[sim]] <- sw2wbb2
  dists_bob_n500[[sim]] <- sw2bob
  
  optimals_x_bob_n500[[sim]] <- bob.out$x.optim
}

results_n500 <- list(
  dist_wbb1 = dists_wbb1_n500,
  dist_wbb2 = dists_wbb2_n500,
  dist_bob = dists_bob_n500,
  opts_x_bob = optimals_x_bob_n500
)

# Export the results -----------------------------------------------------------

save(results_n500, file = "simulations/sim-varying-n/results_n500.Rdata")

...
wbb1Sim.n3 <- vector("list", totalSims)
wbb2Sim.n3 <- vector("list", totalSims)
bobSim.n3  <- vector("list", totalSims)
...
for (i in 1:totalSims) {
  ...
  wbb1Sim.n3[[i]] <- results.wbb1
  wbb2Sim.n3[[i]] <- results.wbb2
  bobSim.n3[[i]]  <- results.bob
  
  rm(wbb1.draws, wbb2.draws, bob.draws, bayes.draws)
  rm(y.new.wbb1, y.new.wbb2, y.new.bob, y.new.bayes)
}

names(wbb1Sim.n3) <- paste0("iteration", 1:totalSims)
names(wbb2Sim.n3) <- paste0("iteration", 1:totalSims)
names(bobSim.n3)  <- paste0("iteration", 1:totalSims)

# Export wbb results
save(wbb1Sim.n3, file = "simulations/sim-varying-n/sim_n3/wbb1Sim_n3.Rdata")
save(wbb2Sim.n3, file = "simulations/sim-varying-n/sim_n3/wbb2Sim_n3.Rdata")
save(bobSim.n3,  file = "simulations/sim-varying-n/sim_n3/bobSim_n3.Rdata")

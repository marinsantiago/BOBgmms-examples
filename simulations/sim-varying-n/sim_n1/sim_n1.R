...
wbb1Sim.n1 <- vector("list", totalSims)
wbb2Sim.n1 <- vector("list", totalSims)
bobSim.n1  <- vector("list", totalSims)
...
for (i in 1:totalSims) {
  ...
  wbb1Sim.n1[[i]] <- results.wbb1
  wbb2Sim.n1[[i]] <- results.wbb2
  bobSim.n1[[i]]  <- results.bob
  
  rm(wbb1.draws, wbb2.draws, bob.draws, bayes.draws)
  rm(y.new.wbb1, y.new.wbb2, y.new.bob, y.new.bayes)
}

names(wbb1Sim.n1) <- paste0("iteration", 1:totalSims)
names(wbb2Sim.n1) <- paste0("iteration", 1:totalSims)
names(bobSim.n1)  <- paste0("iteration", 1:totalSims)

# Export wbb results
save(wbb1Sim.n1, file = "simulations/sim-varying-n/sim_n1/wbb1Sim_n1.Rdata")
save(wbb2Sim.n1, file = "simulations/sim-varying-n/sim_n1/wbb2Sim_n1.Rdata")
save(bobSim.n1,  file = "simulations/sim-varying-n/sim_n1/bobSim_n1.Rdata")

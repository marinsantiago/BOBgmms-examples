...
dists_wbb1_n375 <- dists_wbb2_n375 <- vector("list", total_sims)
dists_bob_n375 <- optimals_x_bob_n375 <- vector("list", total_sims)
...
for (sim in seq_len(total_sims)) {
  ...
  # Store the results
  dists_wbb1_n375[[sim]] <- sw2wbb1
  dists_wbb2_n375[[sim]] <- sw2wbb2
  dists_bob_n375[[sim]] <- sw2bob
  
  optimals_x_bob_n375[[sim]] <- bob.out$x.optim
}

results_n375 <- list(
  dist_wbb1 = dists_wbb1_n375,
  dist_wbb2 = dists_wbb2_n375,
  dist_bob = dists_bob_n375,
  opts_x_bob = optimals_x_bob_n375
)

# Export the results -----------------------------------------------------------

save(results_n375, file = "simulations/sim-varying-n/results_n375.Rdata")

...
dists_wbb1_n50 <- dists_wbb2_n50 <- dists_bob_n50 <- vector("list", total_sims)
optimals_x_bob_n50 <- vector("list", total_sims)
...
for (sim in seq_len(total_sims)) {
  ...
  # Store the results
  dists_wbb1_n50[[sim]] <- sw2wbb1
  dists_wbb2_n50[[sim]] <- sw2wbb2
  dists_bob_n50[[sim]] <- sw2bob
  
  optimals_x_bob_n50[[sim]] <- bob.out$x.optim
}

results_n50 <- list(
  dist_wbb1 = dists_wbb1_n50,
  dist_wbb2 = dists_wbb2_n50,
  dist_bob = dists_bob_n50,
  opts_x_bob = optimals_x_bob_n50
)

# Export the results -----------------------------------------------------------

save(results_n50, file = "simulations/sim-varying-n/results_n50.Rdata")

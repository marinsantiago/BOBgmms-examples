...
wbb1Sim.n5 <- vector("list", totalSims)
wbb2Sim.n5 <- vector("list", totalSims)
bobSim.n5  <- vector("list", totalSims)
...
for (i in 1:totalSims) {
  ...
  wbb1Sim.n5[[i]] <- results.wbb1
  wbb2Sim.n5[[i]] <- results.wbb2
  bobSim.n5[[i]]  <- results.bob
  
  rm(wbb1.draws, wbb2.draws, bob.draws, bayes.draws)
  rm(y.new.wbb1, y.new.wbb2, y.new.bob, y.new.bayes)
}

names(wbb1Sim.n5) <- paste0("iteration", 1:totalSims)
names(wbb2Sim.n5) <- paste0("iteration", 1:totalSims)
names(bobSim.n5)  <- paste0("iteration", 1:totalSims)

# Export wbb results
save(wbb1Sim.n5, file = "simulations/sim-varying-n/sim_n5/wbb1Sim_n5.Rdata")
save(wbb2Sim.n5, file = "simulations/sim-varying-n/sim_n5/wbb2Sim_n5.Rdata")
save(bobSim.n5,  file = "simulations/sim-varying-n/sim_n5/bobSim_n5.Rdata")

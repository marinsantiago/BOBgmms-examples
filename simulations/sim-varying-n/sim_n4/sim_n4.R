...
wbb1Sim.n4 <- vector("list", totalSims)
wbb2Sim.n4 <- vector("list", totalSims)
bobSim.n4  <- vector("list", totalSims)
...
for (i in 1:totalSims) {
  ...
  wbb1Sim.n4[[i]] <- results.wbb1
  wbb2Sim.n4[[i]] <- results.wbb2
  bobSim.n4[[i]]  <- results.bob
  
  rm(wbb1.draws, wbb2.draws, bob.draws, bayes.draws)
  rm(y.new.wbb1, y.new.wbb2, y.new.bob, y.new.bayes)
}

names(wbb1Sim.n4) <- paste0("iteration", 1:totalSims)
names(wbb2Sim.n4) <- paste0("iteration", 1:totalSims)
names(bobSim.n4)  <- paste0("iteration", 1:totalSims)

# Export wbb results
save(wbb1Sim.n4, file = "simulations/sim-varying-n/sim_n4/wbb1Sim_n4.Rdata")
save(wbb2Sim.n4, file = "simulations/sim-varying-n/sim_n4/wbb2Sim_n4.Rdata")
save(bobSim.n4,  file = "simulations/sim-varying-n/sim_n4/bobSim_n4.Rdata")

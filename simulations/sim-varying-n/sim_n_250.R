...
dists_wbb1_n250 <- dists_wbb2_n250 <- vector("list", total_sims)
dists_bob_n250 <- optimals_x_bob_n250 <- vector("list", total_sims)
...
for (sim in seq_len(total_sims)) {
  ...
  # Store the results
  dists_wbb1_n250[[sim]] <- sw2wbb1
  dists_wbb2_n250[[sim]] <- sw2wbb2
  dists_bob_n250[[sim]] <- sw2bob
  
  optimals_x_bob_n250[[sim]] <- bob.out$x.optim
}

results_n250 <- list(
  dist_wbb1 = dists_wbb1_n250,
  dist_wbb2 = dists_wbb2_n250,
  dist_bob = dists_bob_n250,
  opts_x_bob = optimals_x_bob_n250
)

# Export the results -----------------------------------------------------------

save(results_n250, file = "simulations/sim-varying-n/results_n250.Rdata")

...
# ------------------------------------------
# Sim7 - Weighted Bayesian Bootstrap
# ------------------------------------------
...
K <- length(dataSim7$iteration1$params.prior$alphas)

# Initialize data structures to store the WBB output
wbb1Sim7 <- vector("list", totalSims)
wbb2Sim7 <- vector("list", totalSims)
...
for (i in 1:totalSims) {
  ...
  wbb1Sim7[[i]] <- results.wbb1
  wbb2Sim7[[i]] <- results.wbb2
  
  rm(wbb1.draws, wbb2.draws)
  rm(y.new.wbb1, y.new.wbb2)
}

names(wbb1Sim7) <- paste0("iteration", 1:totalSims)
names(wbb2Sim7) <- paste0("iteration", 1:totalSims)

# Export wbb results
save(wbb1Sim7, file = "simulations/sim7/wbb1Sim7.Rdata")
save(wbb2Sim7, file = "simulations/sim7/wbb2Sim7.Rdata")

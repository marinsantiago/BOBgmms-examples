...
# ------------------------------------------
# Sim7 - Bayesian Optimized Bootstrap
# ------------------------------------------
...
for (i in 1:totalSims) {
  ...
  bobSim7[[i]]  <- results.bob
  
  rm(bob.draws, y.new.bob)
}

names(bobSim7) <- paste0("iteration", 1:totalSims)

# Export bob results
save(bobSim7, file = "simulations/sim7/bobSim7.Rdata")

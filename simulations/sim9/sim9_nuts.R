...
#nutsSim9 <- vector("list", totalSims)
adviSim9 <- vector("list", totalSims)
...
for (i in 1:totalSims) {
  ...
  #nutsSim9[[i]] <- results.nuts
  adviSim9[[i]] <- results.advi
  
  #rm(nuts.draws, y.new.nuts)
  rm(advi.draws, y.new.advi)
}

#names(nutsSim9) <- paste0("iteration", 1:totalSims)
names(adviSim9) <- paste0("iteration", 1:totalSims)

# Export NUTS and ADVI results
#save(nutsSim9, file = "simulations/sim9/nutsSim9.Rdata")
save(adviSim9, file = "simulations/sim9/adviSim9.Rdata")

...
nutsSim4 <- vector("list", totalSims)
adviSim4 <- vector("list", totalSims)
...
for (i in 1:totalSims) {
  ...
  nutsSim4[[i]] <- results.nuts
  adviSim4[[i]] <- results.advi
  
  rm(nuts.draws, advi.draws)
  rm(y.new.nuts, y.new.advi)
}

names(nutsSim4) <- paste0("iteration", 1:totalSims)
names(adviSim4) <- paste0("iteration", 1:totalSims)

# Export NUTS and ADVI results
save(nutsSim4, file = "simulations/sim4/nutsSim4.Rdata")
save(adviSim4, file = "simulations/sim4/adviSim4.Rdata")

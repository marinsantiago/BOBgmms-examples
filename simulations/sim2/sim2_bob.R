...
# ------------------------------------------
# Sim2 - Bayesian Optimized Bootstrap
# ------------------------------------------
...
for (i in 1:totalSims) {
  ...
  bobSim2[[i]]  <- results.bob
  
  rm(bob.draws, y.new.bob)
}

names(bobSim2) <- paste0("iteration", 1:totalSims)

# Export bob results
save(bobSim2, file = "simulations/sim2/bobSim2.Rdata")

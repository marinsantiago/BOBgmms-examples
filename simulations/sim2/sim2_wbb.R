...
# ------------------------------------------
# Sim2 - Weighted Bayesian Bootstrap
# ------------------------------------------
...
K <- length(dataSim2$iteration1$params.prior$alphas)

# Initialize data structures to store the WBB output
wbb1Sim2 <- vector("list", totalSims)
wbb2Sim2 <- vector("list", totalSims)
...
for (i in 1:totalSims) {
  ...
  wbb1Sim2[[i]] <- results.wbb1
  wbb2Sim2[[i]] <- results.wbb2
  
  rm(wbb1.draws, wbb2.draws)
  rm(y.new.wbb1, y.new.wbb2)
}

names(wbb1Sim2) <- paste0("iteration", 1:totalSims)
names(wbb2Sim2) <- paste0("iteration", 1:totalSims)

# Export wbb results
save(wbb1Sim2, file = "simulations/sim2/wbb1Sim2.Rdata")
save(wbb2Sim2, file = "simulations/sim2/wbb2Sim2.Rdata")

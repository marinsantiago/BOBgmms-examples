# ------------------------------------------------------------------------------
# Simulations under a trivial initialization (d = 15)
# ------------------------------------------------------------------------------
...
dists_wbb1_d15 <- dists_wbb2_d15 <- dists_bob_d15 <- vector("list", total_sims)
...
for (sim in seq_len(total_sims)) {
  ...
  means.init <- lapply(1:K, \(k) rep(0, d))
  covs.init <- lapply(1:K, \(k) diag(d))
  probs.init <- rep(1/K, K)
  ...
  # Store the results
  dists_wbb1_d15[[sim]] <- sw2wbb1
  dists_wbb2_d15[[sim]] <- sw2wbb2
  dists_bob_d15[[sim]] <- sw2bob
  
}


results_d15 <- list(
  dist_wbb1 = dists_wbb1_d15,
  dist_wbb2 = dists_wbb2_d15,
  dist_bob = dists_bob_d15
)

# Export the results -----------------------------------------------------------

save(results_d15, file = "simulations/sim-trivial-init/results_d15.Rdata")

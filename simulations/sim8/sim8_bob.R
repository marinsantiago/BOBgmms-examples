...
# ------------------------------------------
# Sim8 - Bayesian Optimized Bootstrap
# ------------------------------------------
...
for (i in 1:totalSims) {
  ...
  bobSim8[[i]]  <- results.bob
  
  rm(bob.draws, y.new.bob)
}

names(bobSim8) <- paste0("iteration", 1:totalSims)

# Export bob results
save(bobSim8, file = "simulations/sim8/bobSim8.Rdata")

...
# ------------------------------------------
# Sim8 - Weighted Bayesian Bootstrap
# ------------------------------------------
...
K <- length(dataSim8$iteration1$params.prior$alphas)

# Initialize data structures to store the WBB output
wbb1Sim8 <- vector("list", totalSims)
wbb2Sim8 <- vector("list", totalSims)
...
for (i in 1:totalSims) {
  ...
  wbb1Sim8[[i]] <- results.wbb1
  wbb2Sim8[[i]] <- results.wbb2
  
  rm(wbb1.draws, wbb2.draws)
  rm(y.new.wbb1, y.new.wbb2)
}

names(wbb1Sim8) <- paste0("iteration", 1:totalSims)
names(wbb2Sim8) <- paste0("iteration", 1:totalSims)

# Export wbb results
save(wbb1Sim8, file = "simulations/sim8/wbb1Sim8.Rdata")
save(wbb2Sim8, file = "simulations/sim8/wbb2Sim8.Rdata")

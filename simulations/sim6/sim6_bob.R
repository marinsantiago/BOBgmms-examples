...
# ------------------------------------------
# Sim6 - Bayesian Optimized Bootstrap
# ------------------------------------------
...
for (i in 1:totalSims) {
  ...
  bobSim6[[i]]  <- results.bob
  
  rm(bob.draws, y.new.bob)
}

names(bobSim6) <- paste0("iteration", 1:totalSims)

# Export bob results
save(bobSim6, file = "simulations/sim6/bobSim6.Rdata")

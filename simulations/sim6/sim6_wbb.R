...
# ------------------------------------------
# Sim6 - Weighted Bayesian Bootstrap
# ------------------------------------------
...
K <- length(dataSim6$iteration1$params.prior$alphas)

# Initialize data structures to store the WBB output
wbb1Sim6 <- vector("list", totalSims)
wbb2Sim6 <- vector("list", totalSims)
...
for (i in 1:totalSims) {
  ...
  wbb1Sim6[[i]] <- results.wbb1
  wbb2Sim6[[i]] <- results.wbb2
  
  rm(wbb1.draws, wbb2.draws)
  rm(y.new.wbb1, y.new.wbb2)
}

names(wbb1Sim6) <- paste0("iteration", 1:totalSims)
names(wbb2Sim6) <- paste0("iteration", 1:totalSims)

# Export wbb results
save(wbb1Sim6, file = "simulations/sim6/wbb1Sim6.Rdata")
save(wbb2Sim6, file = "simulations/sim6/wbb2Sim6.Rdata")

...
# ------------------------------------------
# Sim5 - Bayesian Optimized Bootstrap
# ------------------------------------------
...
for (i in 1:totalSims) {
  ...
  bobSim5[[i]]  <- results.bob
  
  rm(bob.draws, y.new.bob)
}

names(bobSim5) <- paste0("iteration", 1:totalSims)

# Export bob results
save(bobSim5, file = "simulations/sim5/bobSim5.Rdata")

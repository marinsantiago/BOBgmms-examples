...
nutsSim5 <- vector("list", totalSims)
adviSim5 <- vector("list", totalSims)
...
for (i in 1:totalSims) {
  ...
  nutsSim5[[i]] <- results.nuts
  adviSim5[[i]] <- results.advi
  
  rm(nuts.draws, advi.draws)
  rm(y.new.nuts, y.new.advi)
}

names(nutsSim5) <- paste0("iteration", 1:totalSims)
names(adviSim5) <- paste0("iteration", 1:totalSims)

# Export NUTS and ADVI results
save(nutsSim5, file = "simulations/sim5/nutsSim5.Rdata")
save(adviSim5, file = "simulations/sim5/adviSim5.Rdata")

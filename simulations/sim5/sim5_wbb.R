...
# ------------------------------------------
# Sim5 - Weighted Bayesian Bootstrap
# ------------------------------------------

set.seed(1)

load("simulations/sim5/dataSim5.Rdata")

totalSims <- length(dataSim5)

K <- length(dataSim5$iteration1$params.prior$alphas)

# Initialize data structures to store the WBB output
wbb1Sim5 <- vector("list", totalSims)
wbb2Sim5 <- vector("list", totalSims)
...
for (i in 1:totalSims) {
  ...
  wbb1Sim5[[i]] <- results.wbb1
  wbb2Sim5[[i]] <- results.wbb2
  
  rm(wbb1.draws, wbb2.draws)
  rm(y.new.wbb1, y.new.wbb2)
}

names(wbb1Sim5) <- paste0("iteration", 1:totalSims)
names(wbb2Sim5) <- paste0("iteration", 1:totalSims)

# Export wbb results
save(wbb1Sim5, file = "simulations/sim5/wbb1Sim5.Rdata")
save(wbb2Sim5, file = "simulations/sim5/wbb2Sim5.Rdata")

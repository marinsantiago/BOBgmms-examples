...
# ------------------------------------------
# Sim3 - Bayesian Optimized Bootstrap
# ------------------------------------------
...
for (i in 1:totalSims) {
  ...
  bobSim3[[i]]  <- results.bob
  
  rm(bob.draws, y.new.bob)
}

names(bobSim3) <- paste0("iteration", 1:totalSims)

# Export bob results
save(bobSim3, file = "simulations/sim3/bobSim3.Rdata")

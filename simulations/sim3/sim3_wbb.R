...
# ------------------------------------------
# Sim3 - Weighted Bayesian Bootstrap
# ------------------------------------------
...
K <- length(dataSim3$iteration1$params.prior$alphas)

# Initialize data structures to store the WBB output
wbb1Sim3 <- vector("list", totalSims)
wbb2Sim3 <- vector("list", totalSims)
...
for (i in 1:totalSims) {
  ...
  wbb1Sim3[[i]] <- results.wbb1
  wbb2Sim3[[i]] <- results.wbb2
  
  rm(wbb1.draws, wbb2.draws)
  rm(y.new.wbb1, y.new.wbb2)
}

names(wbb1Sim3) <- paste0("iteration", 1:totalSims)
names(wbb2Sim3) <- paste0("iteration", 1:totalSims)

# Export wbb results
save(wbb1Sim3, file = "simulations/sim3/wbb1Sim3.Rdata")
save(wbb2Sim3, file = "simulations/sim3/wbb2Sim3.Rdata")

...
nuts_setting4 <- vector("list", total_sims)
advi_setting4 <- vector("list", total_sims)
...
for (sim in seq_len(total_sims)) {
  ...
  if (sum(is.na(nuts.out)) > 0) {
    ...
    dist.nuts <- tryCatch(
      {
        T4transport::swdist(bayes.pred, nuts.pred)$distance
      }, error = function(e) {
        NA
      }
    )
  ...
  if (sum(is.na(advi.out)) > 0) {
    ...
    dist.advi <- tryCatch(
      {
        T4transport::swdist(bayes.pred, advi.pred)$distance
      }, error = function(e) {
        NA
      }
    )
  ...
  nuts_setting4[[sim]] <- results.nuts
  advi_setting4[[sim]] <- results.advi
...
names(nuts_setting4) <- paste0("rep", seq_len(total_sims))
names(advi_setting4) <- paste0("rep", seq_len(total_sims))

# Export the Stan results
folder_path <- "simulations/sim_ar_neg_05/setting4"
save(nuts_setting4, file = paste(folder_path, "/nuts_sim4.Rdata", sep = ""))
save(advi_setting4, file = paste(folder_path, "/advi_sim4.Rdata", sep = ""))

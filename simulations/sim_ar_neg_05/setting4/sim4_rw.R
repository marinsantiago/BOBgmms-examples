# ------------------------------------------------------------------------------
# Sim setting 4: Random weighting
# ------------------------------------------------------------------------------
...
load("simulations/sim_ar_neg_05/setting4/data_setting4.Rdata")

# Simulation settings
total_sims <- length(data_setting4)
K <- length(data_setting4$rep1$params.prior$alphas)
data_dims <- dim(data_setting4$rep1$y)
n <- data_dims[1]
d <- data_dims[2]


# Initialize data structures to store the data
wbb1_setting4 <- vector("list", total_sims)
wbb2_setting4 <- vector("list", total_sims)
bob_setting4 <- vector("list", total_sims)
...
for (sim in seq_len(total_sims)) {
  ...
  wbb1_setting4[[sim]] <- results.wbb1
  wbb2_setting4[[sim]] <- results.wbb2
  bob_setting4[[sim]] <- results.bob
...
names(wbb1_setting4) <- paste0("rep", seq_len(total_sims))
names(wbb2_setting4) <- paste0("rep", seq_len(total_sims))
names(bob_setting4) <- paste0("rep", seq_len(total_sims))

# Export the RW results
folder_path <- "simulations/sim_ar_neg_05/setting4"
save(wbb1_setting4, file = paste(folder_path, "/wbb1_sim4.Rdata", sep = ""))
save(wbb2_setting4, file = paste(folder_path, "/wbb2_sim4.Rdata", sep = ""))
save(bob_setting4, file = paste(folder_path, "/bob_sim4.Rdata", sep = ""))

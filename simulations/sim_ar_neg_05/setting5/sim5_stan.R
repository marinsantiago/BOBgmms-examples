...
nuts_setting5 <- vector("list", total_sims)
advi_setting5 <- vector("list", total_sims)
...
for (sim in seq_len(total_sims)) {
  ...
  nuts_setting5[[sim]] <- results.nuts
  advi_setting5[[sim]] <- results.advi
...
names(nuts_setting5) <- paste0("rep", seq_len(total_sims))
names(advi_setting5) <- paste0("rep", seq_len(total_sims))

# Export the Stan results
folder_path <- "simulations/sim_ar_neg_05/setting5"
save(nuts_setting5, file = paste(folder_path, "/nuts_sim5.Rdata", sep = ""))
save(advi_setting5, file = paste(folder_path, "/advi_sim5.Rdata", sep = ""))

...
nuts_setting3 <- vector("list", total_sims)
advi_setting3 <- vector("list", total_sims)
...
for (sim in seq_len(total_sims)) {
  ...
  nuts_setting3[[sim]] <- results.nuts
  advi_setting3[[sim]] <- results.advi
...
names(nuts_setting3) <- paste0("rep", seq_len(total_sims))
names(advi_setting3) <- paste0("rep", seq_len(total_sims))

# Export the Stan results
folder_path <- "simulations/sim_ar_neg_05/setting3"
save(nuts_setting3, file = paste(folder_path, "/nuts_sim3.Rdata", sep = ""))
save(advi_setting3, file = paste(folder_path, "/advi_sim3.Rdata", sep = ""))

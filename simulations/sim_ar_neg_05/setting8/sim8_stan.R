...
#nuts_setting8 <- vector("list", total_sims)
advi_setting8 <- vector("list", total_sims)
...
for (sim in seq_len(total_sims)) {
  ...
  #nuts_setting8[[sim]] <- results.nuts
  advi_setting8[[sim]] <- results.advi
  
  rm(advi.out, advi.pred)
  gc()
}

#names(nuts_setting8) <- paste0("rep", seq_len(total_sims))
names(advi_setting8) <- paste0("rep", seq_len(total_sims))

# Export the Stan results
folder_path <- "simulations/sim_ar_neg_05/setting8"
#save(nuts_setting8, file = paste(folder_path, "/nuts_sim8.Rdata", sep = ""))
save(advi_setting8, file = paste(folder_path, "/advi_sim8.Rdata", sep = ""))

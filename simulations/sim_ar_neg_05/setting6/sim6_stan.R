...
nuts_setting6 <- vector("list", total_sims)
advi_setting6 <- vector("list", total_sims)
...
for (sim in seq_len(total_sims)) {
  ...
  nuts_setting6[[sim]] <- results.nuts
  advi_setting6[[sim]] <- results.advi
...
names(nuts_setting6) <- paste0("rep", seq_len(total_sims))
names(advi_setting6) <- paste0("rep", seq_len(total_sims))

# Export the Stan results
folder_path <- "simulations/sim_ar_neg_05/setting6"
save(nuts_setting6, file = paste(folder_path, "/nuts_sim6.Rdata", sep = ""))
save(advi_setting6, file = paste(folder_path, "/advi_sim6.Rdata", sep = ""))

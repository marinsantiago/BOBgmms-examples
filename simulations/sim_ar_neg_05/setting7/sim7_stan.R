...
nuts_setting7 <- vector("list", total_sims)
advi_setting7 <- vector("list", total_sims)
...
for (sim in seq_len(total_sims)) {
  ...
  nuts_setting7[[sim]] <- results.nuts
  advi_setting7[[sim]] <- results.advi
...
names(nuts_setting7) <- paste0("rep", seq_len(total_sims))
names(advi_setting7) <- paste0("rep", seq_len(total_sims))

# Export the Stan results
folder_path <- "simulations/sim_ar_neg_05/setting7"
save(nuts_setting7, file = paste(folder_path, "/nuts_sim7.Rdata", sep = ""))
save(advi_setting7, file = paste(folder_path, "/advi_sim7.Rdata", sep = ""))

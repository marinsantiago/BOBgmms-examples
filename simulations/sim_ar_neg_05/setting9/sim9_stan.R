...
#nuts_setting9 <- vector("list", total_sims)
advi_setting9 <- vector("list", total_sims)
...
for (sim in seq_len(total_sims)) {
  ...
  #nuts_setting9[[sim]] <- results.nuts
  advi_setting9[[sim]] <- results.advi
  
  rm(advi.out, advi.pred)
  gc()
}

#names(nuts_setting9) <- paste0("rep", seq_len(total_sims))
names(advi_setting9) <- paste0("rep", seq_len(total_sims))

# Export the Stan results
folder_path <- "simulations/sim_ar_neg_05/setting9"
#save(nuts_setting9, file = paste(folder_path, "/nuts_sim9.Rdata", sep = ""))
save(advi_setting9, file = paste(folder_path, "/advi_sim9.Rdata", sep = ""))

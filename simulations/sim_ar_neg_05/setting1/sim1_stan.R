...
nuts_setting1 <- vector("list", total_sims)
advi_setting1 <- vector("list", total_sims)
...
for (sim in seq_len(total_sims)) {
  ...
  nuts_setting1[[sim]] <- results.nuts
  advi_setting1[[sim]] <- results.advi
...
names(nuts_setting1) <- paste0("rep", seq_len(total_sims))
names(advi_setting1) <- paste0("rep", seq_len(total_sims))

# Export the Stan results
folder_path <- "simulations/sim_ar_neg_05/setting1"
save(nuts_setting1, file = paste(folder_path, "/nuts_sim1.Rdata", sep = ""))
save(advi_setting1, file = paste(folder_path, "/advi_sim1.Rdata", sep = ""))

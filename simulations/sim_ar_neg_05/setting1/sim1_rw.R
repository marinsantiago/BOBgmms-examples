# ------------------------------------------------------------------------------
# Sim setting 1: Random weighting
# ------------------------------------------------------------------------------
...
load("simulations/sim_ar_neg_05/setting1/data_setting1.Rdata")

# Simulation settings
total_sims <- length(data_setting1)
K <- length(data_setting1$rep1$params.prior$alphas)
data_dims <- dim(data_setting1$rep1$y)
n <- data_dims[1]
d <- data_dims[2]


# Initialize data structures to store the data
wbb1_setting1 <- vector("list", total_sims)
wbb2_setting1 <- vector("list", total_sims)
bob_setting1 <- vector("list", total_sims)
...
for (sim in seq_len(total_sims)) {
  ...
  dist.wbb1 <- T4transport::swdist(bayes.pred, wbb1.pred)$distance
  dist.wbb2 <- T4transport::swdist(bayes.pred, wbb2.pred)$distance
  dist.bob <- T4transport::swdist(bayes.pred, bob.pred)$distance
  ...
  wbb1_setting1[[sim]] <- results.wbb1
  wbb2_setting1[[sim]] <- results.wbb2
  bob_setting1[[sim]] <- results.bob
...
names(wbb1_setting1) <- paste0("rep", seq_len(total_sims))
names(wbb2_setting1) <- paste0("rep", seq_len(total_sims))
names(bob_setting1) <- paste0("rep", seq_len(total_sims))

# Export the RW results
folder_path <- "simulations/sim_ar_neg_05/setting1"
save(wbb1_setting1, file = paste(folder_path, "/wbb1_sim1.Rdata", sep = ""))
save(wbb2_setting1, file = paste(folder_path, "/wbb2_sim1.Rdata", sep = ""))
save(bob_setting1, file = paste(folder_path, "/bob_sim1.Rdata", sep = ""))
